...
#      self.points  = np.random.uniform(size = N*self.dimension).reshape(N,self.dimension)
      self.points  = np.array( [ [ 0.42447581,  0.11597012,  0.7700152 ],
                                 [ 0.41151284,  0.07619672,  0.75945154],
                                 [ 0.4117885,   0.04342275,  0.73327324],
                                 [ 0.41757063,  0.06193528,  0.80123613],
                               ] )
      ...
      N = 10
#      self.points = np.random.uniform(size=N*self.dimension).reshape(N,self.dimension)
      self.points = np.array( [ [ 0.44414795,  0.,          0.96082076],
                                [ 0.37631946,  0.01193046,  0.73491831],
                                [ 0.33614742,  0.01510913,  1.        ],
                                [ 0.50011323,  0.05540425,  0.91775411],
                                [ 0.49474115,  0.        ,  0.98120518],
                                [ 0.42956355,  0.1685748 ,  0.71722855],
                                [ 0.55666938,  0.7702964 ,  0.38074925],
                                [ 0.39600869,  0.        ,  0.79259964],
                                [ 0.38855907,  0.00315184,  0.95797447],
                                [ 0.42056186,  0.06533274,  0.78499526],
 ] )
      print " Initial populaion is "
      print np.array([self.map2space(i) for i in self.points])
#      self.fitnesses = np.array([self.fitness(self.map2space(i)) for i in self.points])
      self.fitnesses = np.array( [ 0.25402566,  0.2073158,   0.21666324,  0.13124337,  0.25935698,  0.15802948,  1.47502092,  0.25887712,  0.24320721,  0.0916269837176])
...
## Parameters for NM simplex algorithm      
      ...
      while fit_reflect == "none":
        pnt_reflect = self.space2map(self.fallBack(self.map2space(pnt_reflect)))
...
# =====================================================
    ...
    while self.fitnesses[best_pos] > self.tolerance and self.cost <= 100 :
      self.generation += 1
