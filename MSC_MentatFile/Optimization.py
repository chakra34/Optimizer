...
#      self.points  = np.array([[ 0.33405303,  0.09747169,  0.97415194],
#                               [ 0.34593547,  0.08940729,  0.98604842],
#                               [ 0.30264809,  0.10090849,  0.96979242],
#                               [ 0.33885118,  0.09126603,  0.98719652],
#     ])
      self.points = np.array( [ [ 0.37352365,  0.07932487,  1.        ],
                                [ 0.34593547,  0.08940729,  0.98604842],
                                [ 0.37657836,  0.08452152,  0.99513883],
                                [ 0.33885118,  0.09126603,  0.98719652],
                              ])
      current_fitnesses = []
...
## Parameters for NM simplex algorithm      
      ...
      if fit_reflect < fit_best:                                                                    # new best --> expand simplex further
        ...
        while fit_expansion == "none":
          pnt_expansion = self.space2map(self.fallBack(self.map2space(pnt_expansion)))
          pnt_expansion = self.makeRigid(pnt_expansion) if self.rigid else pnt_expansion
        ...
        while fit_contract == "none":
          pnt_contract = self.space2map(self.fallBack(self.map2space(pnt_contract)))
          pnt_contract = self.makeRigid(pnt_contract) if self.rigid else pnt_contract
        ...
        if fit_contract < fit_worst:                                                              # keep?
          ...
          self.points = self.makeRigid(i for i in self.points) if self.rigid else self.points
          new_fitnesses = []
          for j in rank[1:] :
            curr_fitness = self.fitness(self.map2space(self.points[j]))
            while curr_fitness == "none":
              self.points[j] = self.space2map(self.fallBack(self.map2space(self.points[j])))
...
# =====================================================
    ...
    while self.fitnesses[best_pos] > self.tolerance and self.cost <= 80 :
      self.generation += 1
